...
# Includes HDR support and full GFX12 optimizations
#
...
VARS=(
  ...
  "RADV_DEBUG=nofallback,novrsfl,noinfinitecache,enable_hdr"

  # Расширенные функции GFX12
  "RADV_GFX12_OPTIONS=dual_rt_engines,compact_bvh,hdr_metadata"
  ...
  # Видео и медиа
  "RADV_VIDEO_DECODE=av1,vp9,hevc,avc,hdr"
  "RADV_VIDEO_ENCODE=av1,hdr"
  ...
  "RADV_HDR_SDR_WHITE_LEVEL=300"  # Уровень яркости SDR контента
...
check_system() {
    ...
    if ! grep -q "HDR" /sys/class/drm/card0/edid; then
        echo "⚠️  Внимание: Монитор не поддерживает HDR!"
        echo "HDR переменные будут установлены, но могут не работать"
    fi
...
check_gfx12_support() {
    ...
    echo -e "\nПроверка расширений:"
    vulkaninfo | grep -E \
      "VK_AMD_ray_tracing|VK_KHR_ray_tracing_pipeline|VK_KHR_ray_query|VK_EXT_mesh_shader|VK_EXT_hdr_metadata"
    
    # Проверка возможностей видео
    echo -e "\nВидео возможности:"
    vulkaninfo | grep -E "av1|vp9|hevc|hdr" | grep "decode" | uniq
    
    # Проверка HDR поддержки
    echo -e "\nHDR поддержка:"
    vulkaninfo | grep -E "VK_EXT_swapchain_colorspace|VK_EXT_hdr_metadata"
    echo -e "\nТекущий цветовой профиль:"
    grep "Color Profile" /var/log/Xorg.0.log | tail -1
}
...
show_menu() {
    ...
    echo " 4) Проверить поддержку GFX12 и HDR"
...
check_current_settings() {
    ...
    echo -e "\nАктивные функции:"
    vulkaninfo | grep -E \
      "RADV_PERFTEST|RADV_GFX12|dual_rt_engines|compact_bvh|mesh_shader|hdr_metadata"
    
    # Проверка HDR
    echo -e "\nHDR статус:"
    if grep -q "HDR" /sys/class/drm/card0/edid; then
        echo "✅ Монитор поддерживает HDR"
    else
        echo "⚠️  Монитор не поддерживает HDR"
    fi
